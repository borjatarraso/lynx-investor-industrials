"""Interactive prompt-based mode for lynx-industrials."""
...
from lynx_industrials.core.news import download_article
from lynx_industrials.core.reports import download_filing
from lynx_industrials.display import display_full_report, display_report_stage
from lynx_industrials.models import AnalysisReport

console = Console()

BANNER = "\n[bold blue]  L Y N X   Industrials Analysis[/]\n[dim]    Industrials Fundamental Analysis[/]\n"
...
def run_interactive():
    from lynx_industrials.core.storage import get_mode, is_testing
    ...
    while True:
        ...
        try:
            console.print(f"\n[bold {prompt_color}]lynx-industrials[/] ", end="")
            raw = input().strip()
        ...
        if cmd in ("quit", "exit", "q"):
            ...
            from lynx_industrials import get_about_text; from rich.panel import Panel as P
            a = get_about_text()
            ...
            from lynx_industrials.metrics.explanations import list_metrics
            ...
            if current_report:
                from lynx_industrials.display import _display_business_quality, _display_share_structure, _display_intrinsic_value, _display_market_intelligence
            ...
            if current_report and current_report.filings:
                from lynx_industrials.display import _display_filings; _display_filings(current_report)
            else: console.print("[yellow]No filings available.[/]")
        elif cmd == "news":
            if current_report and current_report.news:
                from lynx_industrials.display import _display_news; _display_news(current_report)
            ...
            from lynx_industrials.core.storage import list_cached_tickers, get_mode
            ...
            from lynx_industrials.easter import rich_matrix; rich_matrix(console)
        elif cmd == "fortune":
            from lynx_industrials.easter import rich_fortune; rich_fortune(console)
        elif cmd == "rocket":
            from lynx_industrials.easter import rich_rocket; rich_rocket(console)
        elif cmd == "lynx":
            from lynx_industrials.easter import rich_lynx; rich_lynx(console)
        else:
