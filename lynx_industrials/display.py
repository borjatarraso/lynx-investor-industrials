"""Rich console display for industrials analysis reports — stage and tier aware.
...
from lynx_industrials.models import AnalysisReport, CompanyStage, CompanyTier, Relevance, Severity
...
# ---------------------------------------------------------------------------
# Assessment functions — all return meaningful text for industrials context
# ---------------------------------------------------------------------------
...
def _a_evrev(val):
    ...
    if val < 3.0:
        return "[yellow]Fair for industrials[/]"
    if val < 6.0:
...
def _a_gm(val):
    ...
    if val < 0.40:
        return "[yellow]Adequate gross margin for industrials[/]"
    if val < 0.55:
...
def _display_header(report):
    ...
    console.print(Panel(
        header,
        title="[bold]LYNX Industrials Analysis[/]",
        border_style="blue",
    ))
...
def _display_profile(report):
    ...
    try:
        from lynx_industrials.metrics.sector_insights import get_sector_insight, get_industry_insight
...
def _display_profitability(report):
    ...
    if all_irrelevant:
        ...
        if p.sga_pct_of_revenue is not None:
            _add_metric_row(t, "SG&A % of Revenue", fmt_pct(p.sga_pct_of_revenue),
                            "[cyan]Overhead intensity — rising % with flat revenue is a margin-leverage warning[/]",
                            Relevance.RELEVANT)
        if p.segment_operating_margin is not None:
            _add_metric_row(t, "Segment Operating Margin (proxy)", fmt_pct(p.segment_operating_margin),
                            "[cyan]Approximate segment-level operating margin before corporate overhead[/]",
                            Relevance.RELEVANT)
        if p.operating_ratio is not None:
            _add_metric_row(t, "Operating Ratio", fmt_pct(p.operating_ratio),
                            f"[{'green' if p.operating_ratio < 0.80 else 'yellow' if p.operating_ratio < 0.90 else 'red'}]"
                            f"{'Best-in-class' if p.operating_ratio < 0.65 else 'Strong' if p.operating_ratio < 0.80 else 'Moderate' if p.operating_ratio < 0.90 else 'Weak'}"
                            " — rails target <62%, LTL <80%[/]",
                            get_relevance("operating_ratio", tier, "profitability", stage))
        if p.fcf_conversion is not None:
            _add_metric_row(t, "FCF Conversion", fmt_pct(p.fcf_conversion),
                            f"[{'green' if p.fcf_conversion > 0.90 else 'yellow' if p.fcf_conversion > 0.60 else 'red'}]"
                            f"{'Excellent capital discipline' if p.fcf_conversion > 0.95 else 'Healthy conversion' if p.fcf_conversion > 0.70 else 'Capex-heavy / working-capital drag'}[/]",
                            get_relevance("fcf_conversion", tier, "profitability", stage))
    console.print(t)
...
def _display_growth(report):
    ...
    if g.capex_intensity is not None:
        ...
        _add_metric_row(t, "Capex Intensity", fmt_pct(g.capex_intensity),
                        ...
                        severity=sev)
    if g.organic_revenue_growth is not None:
        org = g.organic_revenue_growth
        sev = Severity.STRONG if org > 0.05 else Severity.OK if org > 0 else Severity.WATCH if org > -0.05 else Severity.WARNING
        _add_metric_row(t, "Organic Revenue Growth (proxy)", fmt_pct(org),
                        f"[{'green' if org > 0 else 'yellow' if org > -0.05 else 'red'}]"
                        f"{'Existing-base growth outpacing assets' if org > 0.03 else 'Balanced organic / asset growth' if org > -0.02 else 'Growth driven by M&A / asset build'}[/]",
                        get_relevance("organic_revenue_growth", tier, "growth", stage),
                        severity=sev)
    if g.book_to_bill_ratio is not None:
        btb = g.book_to_bill_ratio
        sev = Severity.STRONG if btb > 1.05 else Severity.OK if btb > 0.95 else Severity.WATCH if btb > 0.85 else Severity.WARNING
        _add_metric_row(t, "Book-to-Bill (proxy)", fmt_num(btb, 2),
                        f"[{'green' if btb > 1 else 'yellow' if btb > 0.9 else 'red'}]"
                        f"{'Backlog growing — orders outpacing shipments' if btb > 1.05 else 'Orders tracking shipments' if btb > 0.95 else 'Backlog contracting'}[/]",
                        get_relevance("book_to_bill_ratio", tier, "growth", stage),
                        severity=sev)
    if g.backlog_growth_yoy is not None:
        bg = g.backlog_growth_yoy
        sev = Severity.STRONG if bg > 0.10 else Severity.OK if bg > 0 else Severity.WATCH if bg > -0.10 else Severity.WARNING
        _add_metric_row(t, "Backlog Growth YoY (proxy)", fmt_pct(bg),
                        f"[{'green' if bg > 0 else 'yellow' if bg > -0.10 else 'red'}]"
                        f"{'Forward-revenue visibility expanding' if bg > 0.10 else 'Stable forward visibility' if bg > -0.02 else 'Forward visibility eroding'}[/]",
                        get_relevance("backlog_growth_yoy", tier, "growth", stage),
                        severity=sev)
...
def _display_screening(c):
    ...
    t = Table(title="Industrials Screening Checklist", border_style="cyan")
...
def _display_market_intelligence(report):
    ...
    if mi.disclaimers:
        console.print(Panel(
            ...
            title="[dim]Industrials Disclaimers[/]",
            border_style="dim",
        ))
