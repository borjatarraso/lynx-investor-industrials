"""Command-line interface for lynx-industrials (Industrials Analysis)."""
...
from lynx_industrials import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-industrials",
        description="Lynx Industrials Analysis — fundamental analysis tool for industrial-sector companies.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-industrials -p CAT                           Caterpillar — heavy machinery mega-cap\n"
            "  lynx-industrials -p BA                            Boeing — commercial aerospace OEM\n"
            "  lynx-industrials -p LMT                           Lockheed Martin — defense prime\n"
            "  lynx-industrials -p UNP                           Union Pacific — Class I railroad\n"
            "  lynx-industrials -p UPS --refresh                 United Parcel Service — force fresh data\n"
            '  lynx-industrials -p "Caterpillar"                 Search by name\n'
            "  lynx-industrials -p -i                            Interactive mode\n"
            "  lynx-industrials -t HON                           Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_industrials.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_industrials import get_about_text
    ...
    if "--explain-section" in sys.argv:
        ...
        from lynx_industrials.metrics.explanations import get_section_explanation, SECTION_EXPLANATIONS
    ...
    if "--explain-conclusion" in sys.argv:
        ...
        from lynx_industrials.metrics.explanations import get_conclusion_explanation, CONCLUSION_METHODOLOGY
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_industrials.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_industrials.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_industrials.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_industrials.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_industrials.gui.app import run_gui; run_gui(args); return
    if args.search:
        if not args.identifier: errc.print("[bold red]Error:[/] Provide a search query."); sys.exit(1)
        from lynx_industrials.core.ticker import search_companies, display_search_results
    ...
    from lynx_industrials.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_industrials.export import ExportFormat, export_report
        ...
        from lynx_industrials.core.analyzer import SectorMismatchError
        if isinstance(e, SectorMismatchError):
            ...
            errc.print(Panel(
                ...
                f"[bold red]{e}[/]\n\n"
                f"[bold blink red]This tool is specialized ONLY for Industrials.[/]\n"
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_industrials.core.storage import list_cached_tickers, get_mode
...
def _cmd_drop_cache(con, target):
    from lynx_industrials.core.storage import drop_cache_all, drop_cache_ticker, get_mode
    label = f"({get_mode()})"
    if target.upper() == "ALL":
