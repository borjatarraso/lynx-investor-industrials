...
from lynx_industrials.models import AnalysisReport, CompanyStage, Severity
...
def export_txt(report: AnalysisReport, output_path: Path) -> Path:
    ...
    lines += _header(f"LYNX Industrials  --  {p.name} ({p.ticker})")
    ...
    if report.valuation:
        ...
        _fields = [
            ...
            ("EV per Backlog", _fmt_num(v.ev_per_backlog, 2), Severity.NA),
            ("EV per Employee", _fmt_money(v.ev_per_employee), Severity.NA),
    ...
    if _pre_profit:
        ...
        _fields = [
            ...
            ("Segment Operating Margin (proxy)", _fmt_pct(pr.segment_operating_margin)),
            ("Operating Ratio", _fmt_pct(pr.operating_ratio)),
            ("FCF Conversion", _fmt_pct(pr.fcf_conversion)),
        ]
    ...
    if report.growth:
        ...
        _fields = [
            ...
            ("CAPEX Intensity", _fmt_pct(g.capex_intensity)),
            ("Organic Revenue Growth (proxy)", _fmt_pct(g.organic_revenue_growth)),
            ("Book-to-Bill (proxy)", _fmt_num(g.book_to_bill_ratio, 2)),
            ("Backlog Growth (YoY, proxy)", _fmt_pct(g.backlog_growth_yoy)),
            ("R&D Intensity", _fmt_pct(g.rd_intensity)),
            ("Operating Leverage", _fmt_num(g.operating_leverage, 2)),
        ]
        for label, val in _fields:
            if val != "N/A":
                lines.append(_row(label, val))
    ...
    if report.business_quality:
        ...
        _fields = [
            ...
            ("Backlog / Margin Strength", mq.backlog_strength or "N/A"),
            ("Margin Resilience", mq.margin_resilience or "N/A"),
            ("Unit Economics", mq.unit_economics_quality or "N/A"),
            ("Aftermarket / Services Mix", mq.aftermarket_mix_quality or "N/A"),
            ("Cyclical Sensitivity", mq.cyclical_sensitivity or "N/A"),
            ("End-Market Exposure", mq.end_market_exposure or "N/A"),
    ...
    if c.screening_checklist:
        lines.append("")
        lines.append("  Industrials Screening Checklist:")
        lines.append("  " + "-" * 40)
    ...
    lines.append(f"  Lynx Industrials (Lince Investor Suite)")
    lines.append("=" * W)
